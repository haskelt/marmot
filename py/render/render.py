...
def render_page (file_stem):
    ...
    input_dir = config.constants['content_root']
    output_dir = config.constants['build_root'] + config.arguments['profile']
    root = ET.parse(input_dir + '/' + file_stem + '.xml').getroot()
    template_dirs = ['design/templates']
    env = jinja2.Environment(loader = jinja2.FileSystemLoader(template_dirs))
    root.text = render_node(root, env, dict(), [input_dir])
